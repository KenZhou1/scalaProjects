...
class Game {
  ...
  def checkForBaseDamage(): Unit = {
    ...
    {
      var distance = sqrt(Math.pow(player.location.x - (level.base.x + 0.5),2) + Math.pow(player.location.y - (level.base.y +0.5),2))

      if(playerSize > distance)
      {
        baseHealth -= 1
  ...
  def checkForPlayerHits(): Unit = {
    ...
    for(player <- players.values)
    {
      for(bullets <- projectiles) {
        var distance = sqrt(Math.pow(player.location.x - bullets.location.x, 2) + Math.pow(player.location.y - bullets.location.y, 2))
